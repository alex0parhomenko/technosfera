...
def extract_features(INPUT_FILE_1, INPUT_FILE_2, OUTPUT_FILE):
	...
	for i in range(len(result_url_list)):
		for j in range(len(result_url_list[i])):
			...
			match_with_string.update({"segment_name_" + str(j) + ":" + str(result[i][j]): match_with_string.get("segment_name_" + str(j) + ":" + str(result[i][j]), 0) + 1})
	...
	sorted(result_features_list)
	for i in range(len(result_features_list)):
		f_out.write(result_features_list[i][1] + "\t" + result_features_list[i][0] + "\n")
